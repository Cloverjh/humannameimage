...
function buildThumbnailBackgroundPrompt({
  ...
  return [
    ...
    "- preserve a large clean safe area for an external transparent title PNG",
    "- adapt the safe-area balance to the selected title composition instead of defaulting to centered symmetry",
    "- keep visual interest near edges and corners, using the selected Design DNA graphic language",
    ...
    `Design style family: ${designSpec.designDNA.styleFamily}`,
    `Design composition: ${designSpec.designDNA.composition}`,
    `Graphic language: ${designSpec.designDNA.graphicLanguage}`,
    `Shape language: ${designSpec.designDNA.shapeLanguage}`,
    `Density: ${designSpec.designDNA.density}`,
    `Mood: ${designSpec.designDNA.mood}`,
    ...
    "",
    "Mood:",
    `- follow the selected Design DNA mood: ${designSpec.designDNA.mood}`,
    "- suitable for Human Impact Cooperative education promotion",
    "- visually related to the selected title, but not a generic wellness-brand background",
    "- refined, readable, Canva-friendly",
    "- no excessive visual noise, no neon, no dark heavy background unless explicitly required by the selected palette",
    "",
